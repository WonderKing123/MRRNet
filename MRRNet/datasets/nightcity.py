...
class Nightcity(BaseDataset):
    def __init__(self, 
        ...
        super(Nightcity, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = cv2.imread(os.path.join(self.root,'nightcity',item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape
        ...
        label = cv2.imread(os.path.join(self.root,'nightcity',item["label"]),
            ...
            if scale <= 1.0:
                ...
                for r in range(rows):
                    for c in range(cols):
                        ...
                        pred = self.inference(config, model, crop_img.cuda(), flip)
