...
if __name__ == '__main__':
    ...
    # boundary_outputs main_outputs   aux_outputs
    output = "main_outputs"
    ...
    with torch.no_grad():
        for img_path in images_list:
            ...
            if not boundary:
                ...
                pred = F.interpolate(pred[-1], size=img.size()[-2:],
                                     mode='bilinear', align_corners=True)
                sv_img = pred[0].cpu().detach().numpy().squeeze()
                # fig = plt.figure(figsize=(2048,1024),dpi=1)
                fig = plt.figure(figsize=(2048, 1024),dpi=1)
                plt.imshow(pred[0].cpu().detach().numpy().squeeze())
                if not os.path.exists(sv_path):
                    os.makedirs(sv_path)
                plt.axis('off')
                # plt.savefig(sv_path + img_name)
                plt.savefig(sv_path + img_name, bbox_inches='tight', pad_inches=0)
